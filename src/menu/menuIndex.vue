...
export default {
  ...
  methods: {
    ...
    initAuth() {
      ...
      let authList = [];
      //let routeInfos = this.$router.options.routes.filter(route => {
      //    return route.path.startsWith("/config") || route.path.startsWith("/testplan") || route.path == "/";
      //});
      ...
      authArr.forEach(item => {
        if (item.access) {
          ...
          //routeInfos = routeInfos.filter(route => route.name !== item.id);
        }
      });
      //console.log(routeInfos);
