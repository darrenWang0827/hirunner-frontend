<template>
  <div>
    ...
    <!--this.$router.push没有挂载上组件，可能是这里没有写的原因-->
...
export default {
  ...
  methods: {
    async getTableData() {
      ...
      let url = "/hirunner/plans?" + params.join("&");
    ...
    runPlan(row) {
      ...
      this.$http.post(`/hirunner/plans/${row.id}/run`, params).then(({ data: { msg } }) => {
